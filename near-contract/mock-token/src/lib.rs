...
impl_fungible_token_core!(MockTokenContract, token);
impl_fungible_token_storage!(MockTokenContract, token);

#[near_bindgen]
impl FungibleTokenMetadataProvider for MockTokenContract {
...
mod tests {
    ...
    fn init() -> (VMContextBuilder, AccountId, MockTokenContract) {
        // get VM builer
        ...
        let contract = MockTokenContract::new_default_meta(
    ...
    fn test_transfer() {
        ...
        let mut contract = MockTokenContract::new_default_meta(
            accounts(2),
            String::from("Mock Token"),
            String::from("MOCK"),
            TOTAL_SUPPLY.into(),
